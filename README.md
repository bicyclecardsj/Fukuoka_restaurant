...
- 대용량 CSV는 `.csv.gz`로 압축해 GitHub에 포함하며 Pandas에서 직접 읽습니다.
- GitHub 웹 업로드는 파일 하나당 25 MiB 제한이 있으므로, 압축 후에도 이를 넘는 파일은 Git 명령줄이나 GitHub Desktop으로 push해야 합니다.
- 모델과 tokenizer는 각각 25 MiB보다 작으므로 현재 크기에서는 일반 Git으로 관리할 수 있습니다.
- 공개 저장소에 수집 원본을 추가할 경우 리뷰 작성자 등 불필요한 개인정보를 제거해야 합니다.
